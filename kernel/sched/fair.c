...
// static unsigned long ps_ratio_count = 0;
// static unsigned long lr_ratio_count = 0;
// static unsigned long fast_scan_count = 0;
...
// 	int lr_ratio, ps_ratio;
// 	int diff;
// 	unsigned long prev_scan_period = p->numa_scan_period;
...
// 		/* 스캔 주기 변경 로그 */
// 		pr_info("NUMA: scan period increased (idle/migration failed): %u -> %u ms\n",
// 			prev_scan_period, p->numa_scan_period);
...
// 			slot = 1;
// 		diff = slot * period_slot;
// 		ps_ratio_count++;  // 첫 번째 조건문 실행 카운트 증가
// 	} else if (lr_ratio >= NUMA_PERIOD_THRESHOLD) {
// 		int slot = lr_ratio - NUMA_PERIOD_THRESHOLD;
// 		if (!slot)
// 			slot = 1;
// 		diff = slot * period_slot;
// 		lr_ratio_count++;  // 두 번째 조건문 실행 카운트 증가
// 	} else {
// 		int ratio = max(lr_ratio, ps_ratio);
// 		diff = -(NUMA_PERIOD_THRESHOLD - ratio) * period_slot;
// 		fast_scan_count++;  // 세 번째 조건문 실행 카운트 증가
// 	}
...
// 	/* 스캔 주기 변경 로그 */
// 	pr_info("NUMA: scan period adjusted: %u -> %u ms (diff: %d)\n",
// 		prev_scan_period, p->numa_scan_period, diff);

// 	/* 10초마다 누적 실행 횟수 로그 출력 */
// 	if (time_after(jiffies, last_log_time + msecs_to_jiffies(10000))) {
// 		pr_info("NUMA: Scan condition counts (last 10s) - Local memory: %lu, Shared memory: %lu, Fast scan: %lu\n",
// 			ps_ratio_count, lr_ratio_count, fast_scan_count);
